...
export async function publishedEbooks() { if (!hasDb) return []; return prisma.infoEbook.findMany({ where: { status: "PUBLISHED" }, orderBy: { createdAt: "desc" } }); }
export async function publishedEbook(slug: string) { if (!hasDb) return null; return prisma.infoEbook.findFirst({ where: { slug, status: "PUBLISHED" } }); }
